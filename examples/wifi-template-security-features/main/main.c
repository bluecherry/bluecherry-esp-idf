 ...
 * @author Lucas Musumeci <lucasmusumeci3108@gmail.com>
 ...
 */
#define BLUECHERRY_DEVICE_TYPE "testtest"
...
/**
 * @brief Initialize encrypted NVS.
 *
 * This function will initialize non-volatile storage memory and enable the NVS encryption.
 *
 * @param keys_partition_name The name of the nvs_keys partition
 * @return ESP_OK on success.
 */
 static esp_err_t nvs_encrypted_init(const char *keys_partition_name) {
	if (keys_partition_name == NULL) {
		ESP_LOGE("NVS", "No keys_partition_name specified\nCould not init NVS");
		return ESP_FAIL;
	}
	
 	// Finds the keys partition
 	const esp_partition_t *key_part = esp_partition_find_first(
 		ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_NVS_KEYS,
 		keys_partition_name);

 	if (key_part == NULL) {
 		ESP_LOGE("NVS", "Partition \"%s\" not found in partition table", keys_partition_name);
 		return ESP_FAIL;
 	}

 	// Reads the security configuration (keys) from that partition
 	nvs_sec_cfg_t cfg;
 	esp_err_t err = nvs_flash_read_security_cfg(key_part, &cfg);
 	if (err == ESP_ERR_NVS_KEYS_NOT_INITIALIZED) {
 		// First boot : generates and stores keys
 		ESP_ERROR_CHECK(nvs_flash_generate_keys(key_part, &cfg));
 	} else {
 		ESP_ERROR_CHECK(err);
 	}

	// Uses the SECURE init function instead of the standard one
	esp_err_t ret = nvs_flash_secure_init(&cfg);
	if (ret == ESP_ERR_NVS_NO_FREE_PAGES ||
		ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
		if ((ret = nvs_flash_erase()) != ESP_OK) {
			ESP_LOGE("NVS", "Could not erase NVS: %s", esp_err_to_name(ret));
			return ret;
		}
	...
	if (ret != ESP_OK) {
		ESP_LOGE("NVS", "Could not init NVS: %s", esp_err_to_name(ret));
		return ret;
	}

	ESP_LOGI("NVS", "Initialized encrypted non-volatile storage");
	return ESP_OK;
}
...
void app_main(void)
{
  ESP_LOGI(TAG, "BlueCherry example V1.3.3");

  ESP_ERROR_CHECK(nvs_encrypted_init("nvs_key"));
